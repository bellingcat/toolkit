...
ShadeMap can calculate shadow locations in real time and display them on a map. ShadeMap can also aggregate shadows over time to calculate how many hours of sun or shadow a specific location gets over the course of a day or over the course of an entire year. Sunlight hours and time of year are used to calculate energy values of sunlight per unit area (square metre). In addition to mapping shadow patterns for a static location, ShadeMap's [Trail Shade feature](shademap.md#trail-shade-feature-accessible-from-the-search-bar-menu) handles changing location uploaded as a user trail.
...
1. [Shadow View](shademap.md#view-one-shadow-view)
2. [Hours in the Sun View](shademap.md#view-two-hours-in-the-sun-view)
3. [Annual Sun View](shademap.md#view-three-annual-sunlight-view).
