---
description: >-
  A website that estimates how many of an X/Twitter account’s followers are
  likely real or fake.
---
...
* The [free result is sample‑based;](https://audit.fedica.com/result/pikagambles) it does not analyze every follower. Numbers can be stale if the account has changed since the last audit.&#x20;
* “Fake” is algorithmic shorthand; false positives and false negatives occur. Treat outputs as indicators, not conclusions.
* Private, suspended, or newly created accounts may not be auditable.
* Dependent on X platform policies and access, changes at X can affect coverage or features.
* Some pages are dynamically rendered, which can make archiving or text‑only capture difficult.
