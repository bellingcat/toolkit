...
1. **Steep learning curve:** Maltego is designed for people with a technical background. This background helps the user understand how to manipulate its features and take full advantage of it. It also has a lot of jargon and tech-specific terminology that is difficult to understand. Although, the website describes most of its terminology in the documentation. In addition, transform requests sometimes produce error codes that are not easily understandable. This can be disconcerting to beginner researchers. Understanding the terminology and features requires a lot of time  and perhaps a live or in-person training (which is only available for the paid license and not the free Community Edition).
...
3. **Expensive:** Maltego is pricey, especially for beginner open-source researchers, freelancers, or non-profit organizations.  To fully benefit from its features, users may need additional subscriptions to data sources to equip their investigations with the right set of data. Some Maltego CE users may find that a maximum of 12 transform results per request is too limited to get a good investigative lead. This means the researcher must wait until a month is over to get a new quota on transform requests. A cap on transform results plus a cap on data sources may not provide the desired results. And may, for some, be time-consuming.&#x20;
